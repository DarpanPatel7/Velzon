...
begin
	SELECT AMM.Id,AMM.MenuRank,MReM.Id as MenuResourceId,MReM.MenuName as MenuResourceName,
	AMM.Name,AMM.MenuType,AMM.ParentId ,(select AMMs.name from adminmenumaster as AMMs
...
BEGIN
	...
	FROM 
	(
		-- This subquery uses row_number() to partition by UserId and sort by EntryDatetime in descending order.
		-- It also calculates the maximum EntryDatetime per UserId and assigns conditional lock statuses.
		SELECT DISTINCT     
		   row_number() OVER(PARTITION BY UserId ORDER BY IsLock DESC) srno,
			uld.UserId,
			um.Username,
			MAX(uld.EntryDatetime) AS LastEntryDatetime,
			uld.UpdateBy,
			
			-- Determine LockStatus based on whether the user is locked due to wrong attempts or by admin.
			CASE			
				WHEN uld.IsLock = 1 
					AND uld.UpdateBy IS NULL
					AND MAX(uld.EntryDatetime) >= NOW() - INTERVAL vInterval MINUTE THEN 'Locked by wrong attempts'						
				WHEN uld.IsLock = 1 
					AND uld.UpdateBy IS NOT NULL THEN 'Locked by admin'
				ELSE 'Not Locked'			
			END AS LockStatus,

			-- Assign numeric values to lock status conditions for further processing if needed.
			CASE			
				WHEN uld.IsLock = 1 
					AND uld.UpdateBy IS NULL
					AND MAX(uld.EntryDatetime) >= NOW() - INTERVAL vInterval MINUTE THEN 1						
				WHEN uld.IsLock = 1 
					AND uld.UpdateBy IS NOT NULL THEN 1
				ELSE 0
			END AS IsLock
		   
		FROM userlogdetails uld
		RIGHT JOIN usersmaster um ON um.Id = uld.UserId
		WHERE um.IsActive = 1 
		  AND um.IsDelete = 0 
		  AND um.RoleId <> vSuperAdmin  -- Exclude specific roles based on RoleId.
		GROUP BY uld.UserId, um.Username, IsLock
	) x 

	-- Select only rows where srno is 1, meaning we pick the latest entry for each UserId.
	WHERE srno = 1;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`darpan`@`127.0.0.1` PROCEDURE `cmsInsertOrUpdateAdminMenuMaster`(id int, menuid int, name text, menutype int, parentid int, menurank int, isactive tinyint, changedby text)
begin
	if(id = 0) then
		insert into adminmenumaster(menuid,name,menutype,parentid,menurank,isactive,createby,createdate) values(menuid,name,menutype,parentid,menurank,isactive,changedby,now());
        set id=last_insert_id();
	else
        update adminmenumaster set menuid=menuid,name=name,menutype=menutype,parentid=parentid,menurank=menurank,isactive=isactive,updateby=changedby,updatedate=now() where adminmenumaster.id=id;
    end if;
    select id as recid;
...
update `commoncms_test`.`configdetails` set parametervalue=smtpissecure where parametername='smtpissecure';
else 
insert into `configdetails`
(`parametername`,`parametervalue`,`description`) value 
...
-- Dump completed on 2025-03-13 17:45:20
