...
$(function () {
    ...
    datatable = $.initializeDataTable(tableSelector, { // ✅ Assign to global datatable
        ...
        columns: [
            ...
            { data: "popupDescription", name: "popupDescription", autoWidth: true },
            {
                data: null,
                render: function (data, type, row) {
                    var checked = row.isActive ? "checked" : "";
                    return `<div class="form-check form-switch">
                        <input class="form-check-input toggle-status status${main}" type="checkbox" data-url="${ResolveUrl("/Admin/UpdatePopupStatus")}" data-id="${FrontValue(row.id)}" ${checked}>
            ...
            },
            {
                data: null,
                render: function (data, type, row) {
                    var strEdit = `<a href='javascript:void(0);' class='link-success fs-15 edit${main}' data-url='${ResolveUrl('/Admin/GetPopupDetails')}' data-id='${FrontValue(row.id)}' data-language='${FrontValue(1)}' title='Edit'> <i class='ri-edit-2-line'></i> </a>`;
                    var strRemove = `<a href='javascript:void(0);' class='link-danger fs-15 delete${main}' data-url='${ResolveUrl('/Admin/DeletePopupData')}' data-id='${FrontValue(row.id)}' title='Delete'> <i class='ri-delete-bin-line'></i> </a>`;
    ...
    $(document).on("click", "#add" + main, function () {
        ...
        $.resetForm('#addedit' + main + 'Form', {
            defaultValues: {
                Id: 0,
                Subject: CKEDITOR.instances['popupDescription'].setData("")
            },
        });
    ...
    $(document).on("click", "#addedit" + main + "Submit", function () {
        $("#popupDescription").val(sanitizeCKEditorHTML(CKEDITOR.instances['popupDescription'].getData()));
        if (!ValidateControl($('#popupDescription'))) {
                ...
                if (data.isError == true) {
                    ...
                    Object.keys(dataList).forEach(function (key) {
                        if ($('#' + capitalizeFirstLetter(key)) != null && $('#' + key) != undefined) {
                            ...
                            else if (key == "popupDescription") {
                                CKEDITOR.instances['popupDescription'].setData(dataList[key]);
                    ...
                    CKEDITOR.instances['popupDescription'].setData(dataList[keyName = "popupDescription"]);
    ...
    $("#LanguageId").change(function () {
        if (ValidateControl($("#LanguageId")) && ValidateControl($("#Id"))) {
            ...
            if (intLang > 0 && intId > 0) {
                ...
                $.easyAjax({
                    url: ResolveUrl('/Admin/GetPopupDetails'),
                    type: "POST",
                    data: { "id": encodeURIComponent(id), "langId": encodeURIComponent(langId) }, // Send id in the request body
                    showModal: "#addedit" + main + "Modal",
                    blockUI: true,
