...
namespace Velzon.Webs.Areas.Admin.Controllers
{
    ...
    public class NewsController : BaseController<NewsController>
    {
        ...
        [Route("/Admin/UpdateNewsStatus")]
        [HttpPost]
        public JsonResult UpdateNewsStatus(string id, int isActive)
        {
            JsonResponseModel objreturn = new JsonResponseModel();
            try
            {
                if (long.TryParse(Velzon.Common.Functions.FrontDecrypt(id), out long lgid))
                {
                    ...
                    {
                        objreturn = NewsMasterService.UpdateStatus(lgid, UserModel.Username, isActive);
                    }
                    else
                    {
